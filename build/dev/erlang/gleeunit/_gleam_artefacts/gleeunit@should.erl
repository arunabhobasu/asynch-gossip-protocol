...
-spec be_ok({ok, FDS} | {error, any()}) -> FDS.
be_ok(A) ->
    case A of
        {ok, Value} ->
            Value;
...
-spec be_error({ok, any()} | {error, FDX}) -> FDX.
be_error(A) ->
    case A of
        {error, Error} ->
            Error;
...
-spec be_some(gleam@option:option(FEA)) -> FEA.
be_some(A) ->
    case A of
        {some, Value} ->
            Value;
